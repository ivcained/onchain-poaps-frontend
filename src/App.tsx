...
export function App() {
  ...
  useEffect(() => {
    readTotalEvents().then(setTotalEvents).catch(() => setChainError('Contract read unavailable — check the RPC connection.'))
  }, [])
  ...
  return (
    <main className="app-shell">
      <header className="hero">
        ...
        <div className="hero-row"><div><h1>Turn an idea into a collectible.</h1><p className="intro">Describe the feeling. We’ll draw an editable vector emblem ready for your POAP.</p></div><a className="contract-link" href={explorerAddressUrl} target="_blank" rel="noreferrer">↗ Contract<br /><span>{POAP_CONTRACT_ADDRESS.slice(0, 6)}…{POAP_CONTRACT_ADDRESS.slice(-4)}</span></a></div>
      </header>
      <section className="chain-strip" aria-label="Contract status"><span className="status-dot" /> <strong>Base Sepolia connected</strong><span className="chain-stat">{chainError ?? (totalEvents === null ? 'Reading events…' : `${totalEvents.toString()} events registered`)}</span><span className="chain-id">chain {BASE_SEPOLIA_CHAIN_ID}</span></section>
      <section className="studio" aria-label="SVG generator">
        <div className="preview-panel"><div className="panel-kicker"><span>LIVE PREVIEW</span><span>{svg.length.toLocaleString()} bytes</span></div><div className="art-stage" dangerouslySetInnerHTML={{ __html: svg }} /><div className="preview-actions"><button type="button" onClick={() => setVariation((value) => value + 1)}>↻ New variation</button><button type="button" className="secondary" onClick={downloadSvg}>↓ Download SVG</button></div></div>
        <div className="controls-panel"><label htmlFor="idea">What should this POAP feel like?</label><textarea id="idea" value={idea} onChange={(event) => setIdea(event.target.value)} rows={4} /><label htmlFor="title">Event title on artwork</label><input id="title" value={title} onChange={(event) => setTitle(event.target.value)} maxLength={36} /><fieldset><legend>Mood</legend><div className="choice-row">{moods.map((option) => <button type="button" className={mood === option ? 'choice selected' : 'choice'} onClick={() => setMood(option)} key={option}>{option}</button>)}</div></fieldset><fieldset><legend>Composition</legend><div className="choice-row">{compositions.map((option) => <button type="button" className={composition === option ? 'choice selected' : 'choice'} onClick={() => setComposition(option)} key={option}>{option}</button>)}</div></fieldset><fieldset><legend>Detail</legend><div className="choice-row">{densities.map((option) => <button type="button" className={density === option ? 'choice selected' : 'choice'} onClick={() => setDensity(option)} key={option}>{option}</button>)}</div></fieldset><div className="notice"><strong>Nothing is registered yet.</strong><span>This local preview will become the artwork for the registration transaction in the next step.</span></div></div>
      </section>
      <section className="next-step"><span className="step-number">02</span><div><p className="eyebrow">CONTRACT FOUNDATION</p><h2>Read the chain before we write to it.</h2><p>The app now talks to the deployed Base Sepolia contract and reports the current event count. Registration is the next transaction flow.</p></div></section>
    </main>
  )
}
